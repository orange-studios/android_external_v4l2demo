...
static char             *video_name="video.yuv";
    FILE *fp; //保存视频
...
static void init_device(void) //static表示次函数只能在本文件中调用  
{
...
static void close_device(void)
{
    ...
    if (-1 == close(fd))//==可以防止你写成=（相当与赋值）这样条件始终成立
        errno_exit("close");
...
//关于命令解析见百度getopt-long；
static const char short_options[] = "d:hmruofc:v";// 短选项 ：表示带参数
